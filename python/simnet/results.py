...
def get_avg_vector(config_dir, config_name, vec_name, itvar_name="") -> dict:
...
def get_avg_by_itvars(
    ...
    itvar_avgs = {}
    itvar_values = [
        _df[(_df["attrname"] == itvar_name)]["attrvalue"].drop_duplicates().tolist()
        for itvar_name in itvar_names
    ]
    # itvar_runIds = {} # itvar_comb: runId
    # * just need the vecotrs according to vecname
    _df_veconly = sheet[sheet["name"].str.contains(pat=vec_name, na=False, regex=True)]
    for itvar_comb in itool.product(*itvar_values):
        ...
        for i, itvar_name in enumerate(itvar_names):
            query_index = (_df["attrname"] == itvar_name) & (
                _df["attrvalue"] == itvar_comb[i]
            ) | query_index
        repetition_runIds = _df[query_index]["runID"].drop_duplicates().to_list()
        ...
        itvar_avgs[itvar_comb] = _df_veconly.apply(_calc_mean, axis=1).sum(
            axis=0
        ) / len(repetition_runIds)
